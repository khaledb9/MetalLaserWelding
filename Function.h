...
namespace ATC {
  ...
  public:
    ...
    inline double f(double* x, double t) 
      {return 
			...
			C*exp(( 1.0 - (mask[2]*(x[2]-x0[2])/w)) * ( 1.0 - ( ( (mask[0]*(x[0]-x0[0])*(x[0]-x0[0])) + (mask[1]*(x[1]-x0[1]-(C0*t))*(x[1]-x0[1]-(C0*t))) ) / ( (7.5-(0.2*7.5*mask[2]*(x[2]-x0[2])/w))*(7.5-(0.2*7.5*mask[2]*(x[2]-x0[2])/w)) ) ) ))
  ...
  public:
    ...
    inline double f(double* x, double /* t */) 
      {return  tau*C*(ELECTRON_TEMPERATURE-C0);};
